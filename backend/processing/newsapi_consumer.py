...
GROUP_NAME = "newsapi_sentiment-processors"
...
async def ensure_consumer_group(redis):
    ...
    try:
        ...
        if "BUSYGROUP" in str(e):
            print(f"Consumer group '{STREAM_NAME}' already exists")
        else:
            raise
...
async def run():
    ...
    while True:
        try:
            ...
            if not messages:
                await asyncio.sleep(3600)  # chil for an hr when nothing to process
                continue
